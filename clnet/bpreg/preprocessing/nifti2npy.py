...
class Nifti2Npy:
    ...
    def preprocess_npy_with_spacing(self, filepath: str):
        # convert the simpleitk data matrix to nibabel data matrix direction
        x = np.transpose(np.load(filepath)[0], (2, 1, 0))
        pixel_spacings = json.load(open(filepath[:-len("npy")] + "json"))["itk_spacing"]
        x = self.rescale_xy(x)
        x = self.resize_volume(x, pixel_spacings)
        return x, pixel_spacings
