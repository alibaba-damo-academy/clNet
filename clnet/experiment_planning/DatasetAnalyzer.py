...
from clnet.preprocessing.cropping import get_patient_identifiers_from_cropped_files
...
class DatasetAnalyzer(object):
    ...
    def _compute_stats(voxels):
        ...
        voxels = voxels[np.where(np.logical_and(voxels > percentile_00_5, voxels < percentile_99_5))]
        if len(voxels) == 0:
            return median, mean, sd, mn, mx, percentile_99_5, percentile_00_5
        median_ = np.median(voxels)
        mean_ = np.mean(voxels)
        sd_ = np.std(voxels)
        mn_ = np.min(voxels)
        mx_ = np.max(voxels)
        percentile_99_5_ = np.percentile(voxels, 99.5)
        percentile_00_5_ = np.percentile(voxels, 00.5)

        sd_max = max(sd, sd_)
        sd_min = min(sd, sd_)
        if isinstance(sd_max, (np.float64, np.float32, np.float16, np.int32, np.int16, np.int8)) and \
                isinstance(sd_min, (np.float64, np.float32, np.float16, np.int32, np.int16, np.int8)) and sd_min != 0:
            if sd_max / sd_min > 2:
                return median_, mean_, sd_, mn_, mx_, percentile_99_5_, percentile_00_5_
        return median, mean, sd, mn, mx, percentile_99_5, percentile_00_5
    ...
    def collect_intensity_properties(self, num_modalities):
        if self.overwrite or not isfile(self.intensityproperties_file):
            ...
            for mod_id in range(num_modalities):
                ...
                v = p.starmap(self._get_voxels_in_foreground, zip(self.patient_identifiers, [mod_id] * len(self.patient_identifiers)))
