...
from clnet.preprocessing.preprocessing import get_lowres_axis, get_do_separate_z, resample_data_or_seg
...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(shape_original_after_cropping))]):
        ...
        seg_old_spacing = resample_data_or_seg(segmentation_softmax, shape_original_after_cropping, is_seg=False,
                                               axis=lowres_axis, do_separate_z=do_separate_z)
    ...
    if np.any(np.array(current_shape) != np.array(shape_original_after_cropping)):
        if order == 0:
            ...
            seg_old_spacing = resample_data_or_seg(segmentation[None], shape_original_after_cropping, is_seg=True,
                                                   axis=lowres_axis, do_separate_z=do_separate_z)[0]
    else:
...
def save_and_resample_segmentation_nifti(segmentation, out_fname, dct):
    # suppress output
    # print("force_separate_z:", force_separate_z, "interpolation order:", order)
    # sys.stdout = open(os.devnull, 'w')
    ...
    # first resample, then put result into bbox of cropping, then save
    current_shape = segmentation.shape
    shape_original_after_resample = dct.get('resampled_size_of_raw_data')
    assert np.any(np.array(current_shape) == np.array(shape_original_after_resample))
    out_fname = out_fname[:-len(".nii.gz")] + ".npy"
    segmentation = np.array(segmentation, dtype=np.uint16)
    np.save(out_fname, segmentation)
    # shape_original_before_resample = dct.get('original_size_of_raw_data')
    # if np.any(np.array(current_shape) != np.array(shape_original_before_resample)):
    #     if order == 0:
    #         seg_old_spacing = resize_segmentation(segmentation, shape_original_before_resample, 0)
    #     else:
    #         # if force_separate_z is None:
    #         #     if get_do_separate_z(dct.get('original_spacing')):
    #         #         do_separate_z = True
    ...
    #
    #         # seg_old_spacing = F.interpolate(segmentation[None][None], tuple(shape_original_before_resample), mode="nearest")[0][0]
    #         # if device != "cpu":
    #         #     seg_old_spacing = seg_old_spacing.detach().cpu().numpy()
    # else:
    #     seg_old_spacing = segmentation

    seg_resized_itk = sitk.GetImageFromArray(segmentation.astype(np.uint16))
    seg_resized_itk.SetSpacing(dct['resampled_itk_spacing'])
    seg_resized_itk.SetOrigin(dct['itk_origin'])
    seg_resized_itk.SetDirection(dct['itk_direction'])
    sitk.WriteImage(seg_resized_itk, out_fname)
    # del seg_old_spacing
