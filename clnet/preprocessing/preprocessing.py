...
from clnet.configuration import default_num_threads, RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD, default_ct_intensity_properties
from clnet.preprocessing.cropping import get_case_identifier_from_npz, ImageCropper
...
def resample_patient(data, seg, original_spacing, target_spacing, order_data=3, order_seg=0, force_separate_z=False, order_z_data=0,
                     order_z_seg=0, separate_z_anisotropy_threshold=RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD, scheme=None):
    """
    :param data:
    :param seg:
    ...
    :param separate_z_anisotropy_threshold: if 3max_spacing > separate_z_anisotropy_threshold * min_spacing (per axis)
    ...
    if data is not None:
        if len(idx_sup) > 0:
            data_sup = data[idx_sup, ::-1]
            data_reg = data[idx_reg, ::-1]
            data_reshaped = np.zeros(tuple([data.shape[0]] + list(new_shape)), dtype=data.dtype)
            data_reshaped[idx_sup, ::-1] = resample_data_or_seg(data_sup, new_shape, True, axis, do_separate_z)
            data_reshaped[idx_reg, ::-1] = resample_data_or_seg(data_reg, new_shape, False, axis, do_separate_z)
        else:
...
def resample_torch_simple(
    ...
    if mode == 'linear':
        ...
        torch_mode = 'nearest' if is_seg else 'bilinear'
        ...
        with torch.no_grad():
            ...
            if is_seg:
                ...
                if not memefficient_seg_resampling:
                    ...
                    for i, u in enumerate(unique_values):
                        result_tmp[i] = \
                            F.interpolate((data[None] == u).float() * scale_factor, new_shape, mode=torch_mode, align_corners=False)[0]
                    ...
                    for i, u in enumerate(unique_values):
                        if u == 0:
                            pass
                        result[F.interpolate((data[None] == u).float(), new_shape, mode=torch_mode)[0] > 0.5] = u
            else:
                result = F.interpolate(data[None].float(), new_shape, mode=torch_mode, align_corners=False)[0]
            if input_was_numpy:
...
def resample_data_or_seg_speedup(data, new_shape, is_seg, axis=None, do_separate_z=False):
    """
    ...
    if do_separate_z:
        ...
        data = resample_torch_simple(data, tmp_new_shape, is_seg=is_seg, memefficient_seg_resampling=memefficient_seg_resampling, mode=mode)
        ...
        data = resample_torch_simple(data, new_shape, is_seg=is_seg, memefficient_seg_resampling=memefficient_seg_resampling, mode=mode)

        if was_numpy:
            data = data.numpy()
        return data
    else:
        return resample_torch_simple(data, new_shape, is_seg, memefficient_seg_resampling)


def resample_data_or_seg(data, new_shape, is_seg, axis=None, do_separate_z=False, order=3, order_z=0):
    """
    separate_z=True will resample with order 0 along z
    :param data:
    :param new_shape:
...
class GenericPreprocessor(object):
    def __init__(self, normalization_scheme_per_modality, use_nonzero_mask, transpose_forward: (tuple, list), intensity_properties: dict = None):
        """
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        properties["original_size_of_raw_data_cropped"] = data[0].shape
        ...
        for c in range(len(data)):
            ...
            if "ct" in scheme.lower() and "nonct" not in scheme.lower():
    ...
    def preprocess_test_case(self, data_files, target_spacing, bm_file=None, bpr_range=None, force_separate_z=None, patch_size=None):
        data, _, properties = ImageCropper.crop_from_list_of_files_body_mask(data_files, bm_file, bpr_range, patch_size)

        data = data.transpose((0, *[i + 1 for i in self.transpose_forward]))

        data, bm, properties = self.resample_and_normalize(data, target_spacing, properties, force_separate_z=force_separate_z)
        return data.astype(np.float32), bm, properties

    def _run_internal(self, target_spacing, case_identifier, output_folder_stage, cropped_output_dir,
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        for i in range(num_stages):
            ...
            list_of_cropped_npz_files_remained = [i for i in list_of_cropped_npz_files if
                                                  (not os.path.isfile(os.path.join(output_folder_stage, os.path.basename(i)))) or
                                                  (not os.path.isfile(os.path.join(output_folder_stage, os.path.basename(i).replace(".npz", ".pkl"))))]
            print("Processing {} files in stage {}.".format(len(list_of_cropped_npz_files_remained), i))
            spacing = target_spacings[i]
            for j, case in enumerate(list_of_cropped_npz_files_remained):
