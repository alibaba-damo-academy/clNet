...
class DownsampleSupportDataForDSTransform(AbstractTransform):
    ...
    '''
    def __init__(self, ds_scales=(1, 0.5, 0.25), input_key="data", output_key="aux_data"):
        self.output_key = output_key
        self.input_key = input_key
        self.ds_scales = ds_scales

    def __call__(self, **data_dict):
        data_dict[self.output_key] = downsample_support_for_ds_transform(data_dict[self.input_key][:, 1:], self.ds_scales)
        return data_dict

def downsample_support_for_ds_transform(aux_data, ds_scales=((1, 1, 1), (0.5, 0.5, 0.5), (0.25, 0.25, 0.25))):
    output = []
    ...
    for s in ds_scales:
        if all([i == 1 for i in s]):
            ...
            pooled = pool_op(aux_data_torch, kernel_size, stride, pad, count_include_pad=False, ceil_mode=False)
