...
def augment_spatial_v2(data, seg, patch_size, patch_center_dist_from_border=30,
                       ...
                       scheme=None, dummy_2d=None, orig_shape_data=None):
    dim = len(patch_size)
    seg_result = None
    if seg is not None:
        if dim == 2:
            ...
            if dummy_2d is not None and orig_shape_data is not None and dummy_2d:
                for channel_s in range(orig_shape_data[1]):
                    if scheme is not None and scheme[channel_s] == "support":
                        for channel_id in range(orig_shape_data[2]):
                            data_result[sample_id, channel_s * orig_shape_data[2] + channel_id] = \
                                interpolate_img(data[sample_id, channel_s * orig_shape_data[2] + channel_id],
                                                coords, order_seg, border_mode_seg,
                                                cval=border_cval_seg, is_seg=True)
                    else:
                        for channel_id in range(orig_shape_data[2]):
                            data_result[sample_id, channel_s * orig_shape_data[2] + channel_id] = \
                                interpolate_img(data[sample_id, channel_s * orig_shape_data[2] + channel_id],
                ...
                for channel_id in range(data.shape[1]):
                    if scheme is not None and scheme[channel_id] == "support":
                        data_result[sample_id, channel_id] = interpolate_img(data[sample_id, channel_id],
                                                                             coords, order_seg, border_mode_seg,
                                                                             cval=border_cval_seg, is_seg=True)
                    else:
                        data_result[sample_id, channel_id] = interpolate_img(data[sample_id, channel_id],
