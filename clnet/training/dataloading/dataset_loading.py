...
from clnet.paths import preprocessing_output_dir
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent: tuple = (0.0, 0.0), memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None, oversample_foreground_class=None, oversample_percent_decay: bool = True,
                 head_info=None, train_dict=None, max_num_batches=None, curr_batch_idx=None):
        """
        ...
        self.ofp_range = oversample_foreground_percent
        self.ofp = oversample_foreground_percent[0]
        self.oversample_foreground_class = oversample_foreground_class
        self.oversample_percent_decay = oversample_percent_decay
        ...
        self.get_do_oversample = self._oversample_last_xx_percent  # if not probabilistic_oversampling else self._probabilistic_oversampling
        assert head_info is not None and len(head_info) == 3
        assert train_dict is not None
        self.head_to_train = head_info[0]
        self.decoder_or_support = head_info[1]
        self.num_classes = head_info[2]
        self.train_dict = train_dict
        self.max_num_batches = max_num_batches
        self.batch_idx = curr_batch_idx
    ...
    def _oversample_last_xx_percent(self, sample_idx: int) -> bool:
        ...
        return not sample_idx < round(self.batch_size * (1 - self.ofp))

    def _probabilistic_oversampling(self, sample_idx: int) -> bool:
        # return not batch_idx < round(self.batch_size * (1 - self.oversample_foreground_percent))
        return np.random.uniform() < self.ofp

    def maybe_update_ofp(self):
        if self.oversample_percent_decay:
            self.ofp = \
                max(self.ofp_range[1], 1.0 * self.ofp_range[0] - ((self.ofp_range[0] - self.ofp_range[1]) / max(1, self.max_num_batches)) * self.batch_idx)
    ...
    def _gen_new_label(self, target, task_classes):
        if not isinstance(task_classes, list):
            task_classes = [task_classes]
        if self.num_classes == len(task_classes):
            return target
        # find the consecutive subsequence lists
        k0 = task_classes[0]
        sections = []
        sub_section = [k0]
        for k1 in task_classes[1:]:
            if k1 - 1 == k0:
                sub_section.append(k1)
                k0 = k1
            else:
        ...
        if len(sub_section) > 0:
            sections.append(sub_section)
        # Get each subsequence's offset
        section_offset = [0]
        for section in sections:
            section_offset.append(len(section))
        section_offset = [sum(section_offset[:y]) for y in range(1, len(section_offset) + 1)]
        section_offset = section_offset[:len(sections)]
        ret_target = np.zeros_like(target)
        for s, section in enumerate(sections):
            target_copy = target.copy()
            idx_min = section[0]
            idx_max = section[-1]
            current_offset = section_offset[s] - idx_min + 1
            target_copy += current_offset
            target_copy[target_copy > idx_max + current_offset] = 0
            if current_offset != 0:
                target_copy[target_copy < idx_min + current_offset] = 0
            ret_target += target_copy
        return ret_target

    def reorder_label(self, target, head_to_train):
        if head_to_train == "all":
            head_list = list(self.train_dict["decoders"].keys()) if self.decoder_or_support == "decoders" else \
                list(self.train_dict["supporting"].keys())
            new_seg_shape = (self.batch_size, len(head_list), *self.patch_size)
            ret_target = np.zeros(new_seg_shape, dtype=np.float32)
            for idx, head in enumerate(head_list):
                ret_target[:, [idx], :] = self._gen_new_label(target, self.train_dict["decoders"][head])
        else:
            # If we only update "specific" decoding/supporting head for training,
            # we do not need to differentiate whether it is from the decoding head or supporting
            ret_target = self._gen_new_label(target, self.train_dict["decoders"][head_to_train])
        return ret_target
    ...
    def generate_train_batch(self):
        ...
        case_properties = []
        # total_cases = 0
        for k, i in enumerate(selected_keys):
            # oversampling foreground will improve stability of model training, especially if many patches are empty
            # (Lung for example)
            if self.get_do_oversample(k):
                force_fg = True
            else:
                force_fg = False
            ...
            # concatenate it. Here it will be concatenated to the segmentation because the augmentations need to be
            ...
            for d in range(3):
                # if case_all_data.shape + need_to_pad is still < patch size we need to pad more!
            ...
            if not force_fg:
                ...
                if 'class_locations' not in properties.keys():
                    raise RuntimeError("Please rerun the preprocessing with the newest version of clNet!")
                ...
                if self.oversample_foreground_class is not None:
                    foreground_classes = list(set(foreground_classes) & set(self.oversample_foreground_class))

                if len(foreground_classes) == 0:
                    # this only happens if some image does not contain foreground voxels at all
                    voxels_of_that_class = None
                    # print('Case {} does not contain any foreground classes'.format(i))
                else:
            ...
            if seg_from_previous_stage is not None:
                seg_from_previous_stage = \
                    seg_from_previous_stage[:, valid_bbox_x_lb:valid_bbox_x_ub, valid_bbox_y_lb:valid_bbox_y_ub, valid_bbox_z_lb:valid_bbox_z_ub]

            data[k] = np.pad(
                case_all_data[:-1], ((0, 0), (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)), (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                     (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))), self.pad_mode, **self.pad_kwargs_data)

            seg[k, 0] = np.pad(
                case_all_data[-1:], ((0, 0), (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)), (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                     (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))), 'constant', **{'constant_values': -1})
            if seg_from_previous_stage is not None:
                seg[k, 1] = np.pad(
                    seg_from_previous_stage, ((0, 0), (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)), (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                              (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))), 'constant', **{'constant_values': 0})

        seg = self.reorder_label(seg, head_to_train=self.head_to_train)
        self.maybe_update_ofp()
        if self.batch_idx is not None:
            self.batch_idx += 1
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'ofp': self.ofp}
